...
void XmlRepository::saveAnnotation(const std::shared_ptr<Annotation>& annotation, pugi::xml_node* node) const
{
    ...
    pugi::xml_node nodeTumorT = nodeAnnotation.append_child("TumorTypes");
    std::set<std::string> tumorTypes = annotation->getTumorTypes();
    for (std::set<std::string>::const_iterator it = tumorTypes.begin(); it != tumorTypes.end(); ++it) {
        pugi::xml_node nodeTumorType = nodeTumorT.append_child("TumourType");
        pugi::xml_attribute attributeType = nodeTumorType.append_attribute("Type");
        attributeType.set_value((*it).c_str());
    }
}
...
bool XmlRepository::loadFromRepo()
{
    ...
    for (pugi::xml_node_iterator it = annotations.begin(); it != annotations.end(); ++it)
    {
        ...
        //////////////////////////////////////////////////////////////////////////
        //Tumor types
        pugi::xml_node tumourTypes = it->child("TumorTypes");
        for (pugi::xml_node_iterator cit = tumourTypes.begin(); cit != tumourTypes.end(); ++cit)
        {
            pugi::xml_attribute attributeType = cit->attribute("Type");
            std::cout << attributeType.as_string() << std::endl;
            annotation->addTumorType(attributeType.as_string());
        }
